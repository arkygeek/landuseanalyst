...
\section{Calorie Targets}
  \index{Landuse Analyst!Calorie Targets}
  Calorie Targets are the first level of calculations done by Landuse Analyst.  Calorie targets are obtained through a number of steps (see Fig. \ref{fig:dietDiagram}), which are as follows.
...
\begin{figure}[ht] %dietMeatDiagram
 ...
 % dietDiagram.jpg: 740x731 pixel, 72dpi, 26.10x25.78 cm, bb=0 0 740 731
 \textit{\caption[Plant Portion of Diet]{\label{fig:dietMeatDiagram}How LA sees the meat portion of the diet.}}
...
\begin{figure}[ht] %dietCropDiagram
 ...
 \textit{\caption[Plant Portion of Diet]{\label{fig:dietCropDiagram}How LA sees the plant portions of the diet.  Note that the input figures for percent of diet were: \textbf{HorseBean} $7\%$, \textbf{Einkorn} $13\%$, \textbf{Emmer} $34\%$, \textbf{Lentils} $12\%$, \textbf{Peas} $15\%$, \textbf{Olives}\ $18\%$.  These numbers translate to the ones shown above after processing as in Fig. \ref{fig:dietDiagram}.}}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{ Diet Tab, Landuse Analyst}
\end{figure}

\section{Production Targets}
  \index{Landuse Analyst!Production Targets}
...
\index{Landuse Analyst!Area Targets}
...
hand corner of the form.
    ...
    \begin{figure}[htbp]
      ...
      \label{fig:crop} \caption{Crops Tab, Landuse Analyst Version 0.1 Revision 544}
    \end{figure}
    
  \textbf{Crop rotation}\index{Landuse Analyst!Crop Rotation} affects the area target directly, so a few points about it need to be
mentioned.  Quite simply, Landuse Analyst  treats crop rotation as a ratio of sown land to fallow land.  If a ratio of $1:1.00$ is used,
...
value with the keyboard.
  ...
  \subsection{Animals}
  Herd Size\index{Landuse Analyst!Grazing Requirements} determination is perhaps the single most difficult calculation that Landuse Analyst
performs.  Added to the complexity of this is the problem that different animals have different dietary requirements, and even further, even if they are the same, they require different amounts of feed depending on things like whether they are pregnant or lactating, and what the weather conditions are like.  Again, to further complicate this issue, not all land that can be grazed has the same food value to the animals.
  ...
  With the exception of weather conditions, Landuse Analyst addresses all of the issues.  One of the first things a user must do when using
