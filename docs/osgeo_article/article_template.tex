...
\end{figure}
  ...
  \subsection{Animals}
    ...
    LA does, however, address these issues.  One of the first things a user must do
    ...
    \subsubsection{Animal Characteristics}
      ...
      \begin{itemize}
        \item \textit{\textbf{Common Land}} - Sometimes land is suitable for
          ...
          can be accomplished by removing the other animals one at a time from using
...
\section{Finding the land} 
  ...
  approach.  Instead of using a step amount, a percentage is used.  This percentage
  value is called the \textit{Precision}.  
  
  The area of land deemed by LA as equalling the current area target gets changed into
