...
def update_animals_xml(filepath, animal_updates):
    ...
    for animal in root.findall('animal'):
        guid = animal.get('guid')
        if guid in animal_updates:
            updates = animal_updates[guid]
            print(f"Updating animal {guid} ({updates.get('name', 'unknown')})")
            for tag, value in updates.items():
                elem = animal.find(tag)
                if elem is not None:
                    elem.text = str(value)
                else:
                    new_elem = ET.SubElement(animal, tag)
...
update_animals_xml(target_file, animal_updates)
